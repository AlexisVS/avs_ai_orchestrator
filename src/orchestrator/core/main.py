...
class MainOrchestrator:
    ...
    def __init__(self, config_path: str):
        ...
        self.evolution_agent = SelfEvolutionAgent(self.universal_orchestrator.config)
        self.auto_evolution_enabled = self.universal_orchestrator.config.get("auto_evolution", {}).get("enabled", False)
        
        print("[MAIN] Orchestrateur principal initialisé")
        if self.auto_evolution_enabled:
            print("[MAIN] Auto-évolution activée")
    ...
    async def run_full_workflow(self):
        ...
        try:
            ...
            # 7. Démarrer l'auto-évolution si activée
            if self.auto_evolution_enabled:
                print("\n[AUTO-EVOLUTION] Démarrage de la boucle d'auto-évolution")
                asyncio.create_task(self.evolution_agent.start_evolution_loop())
            
            print("\n" + "=" * 80)
            print("[SUCCESS] ORCHESTRATION TERMINÉE AVEC SUCCÈS!")
            if self.auto_evolution_enabled:
                print("[INFO] Boucle d'auto-évolution démarrée en arrière-plan")
            print("=" * 80)
